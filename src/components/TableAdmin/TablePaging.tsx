...
function TablePaging(props: ITablePaging) {
    ...
    useEffect(() => {
        // Adjust the page range when pageActive changes
        const { pageActive } = props;
        const startPage = Math.floor((pageActive as number - 1) / 10) * 10 + 1;
        const endPage = Math.min(startPage + 9, props.data.data.totalPage);
        setPageRange([startPage, endPage]);
    }, [props.pageActive, props.data.data.totalPage]);
                ...
                <div className="flex mt-2 sm:mt-auto sm:justify-end">
                    <nav aria-label="Product Page Navigation">
                        <ul className="inline-flex items-center">
                            ...
                            {props?.data?.data?.totalPage >= 2 &&
                                Array.from({ length: Math.min(10, props.data.data.totalPage) }).map(
                                    (_, i) => {
                                        const pageNum = pageRange[0] + i;
                                        return (
                                            <li
                                                key={pageNum}
                                                className={`${pageNum === props.pageActive
                                            ...
                                            >
                                                <button
                                                    ...
                                                    type="button"
                                                    onClick={() => props.setPageActive && props.setPageActive(pageNum)}
                                                >
                                                    {pageNum}
                                                </button>
                                            </li>
