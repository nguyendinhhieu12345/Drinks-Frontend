...
export default function Coupons() {
    ...
    const handleRedirectEditCoupon = (coupon: {
        id: string;
        code: string;
        couponType: string;
        status: string;
        ...
        if (coupon.couponType === "ORDER") {
            nav(configRouter?.couponAmountOfOrderEdit.slice(0, -3) + coupon.id)
        }
        else if (coupon.couponType === "SHIPPING") {
            nav(configRouter?.couponShippingEdit.slice(0, -3) + coupon.id)
        }
        else {
            if (coupon?.rewardType === "PRODUCT_GIFT") {
                nav(configRouter?.couponBuyXGetYEdit.slice(0, -3) + coupon.id)
            }
            else {
                nav(configRouter?.couponAmountOfProductEdit.slice(0, -3) + coupon.id)
            }
        }
    }
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="flex justify-end text-right">
                                            <button
                                                ...
                                                onClick={() => handleRedirectEditCoupon(coupon)}
                                            >
                                                <p
                                                    data-tip="true"
                                                    data-for="delete"
